...
import byou.yadun.wallet.Chat.*;
import byou.yadun.wallet.db.DemoDBManager;
import byou.yadun.wallet.db.InviteMessgeDao;
import byou.yadun.wallet.db.UserDao;
import byou.yadun.wallet.domain.EmojiconExampleGroupData;
import byou.yadun.wallet.domain.InviteMessage;
import byou.yadun.wallet.domain.RobotUser;
import byou.yadun.wallet.parse.UserProfileManager;
import byou.yadun.wallet.receiver.CallReceiver;
import byou.yadun.wallet.utils.PreferenceManager;
import byou.yadun.wallet.utils.PreferenceUtil;
import byou.yadun.wallet.utils.SPUtils;
...
public class ByHelper {
    ...
    protected static final String TAG = "ByHelper";
    ...
    private static ByHelper instance = null;

    private ByModel demoModel = null;
    ...
    public synchronized static ByHelper getInstance() {
        if (instance == null) {
            instance = new ByHelper();
        }
        return instance;
    }

    private final String[] avatar = {"http://api.ydchain.cc/app/ic1.png",
...
//        options.setImPort(31097);
            ...
            public Intent getLaunchIntent(EMMessage message) {
                ...
                if (isVideoCalling) {
                    ...
                    if (chatType == ChatType.Chat) { // single chat message
                        ...
                        intent.putExtra("chatType", byou.yadun.wallet.Chat.Constant.CHATTYPE_SINGLE);
                    } else { // group chat message
                        // message.getTo() is the group id
                        intent.putExtra("userId", message.getTo());
                        if (chatType == ChatType.GroupChat) {
                            intent.putExtra("chatType", byou.yadun.wallet.Chat.Constant.CHATTYPE_GROUP);
                        } else {
                            intent.putExtra("chatType", byou.yadun.wallet.Chat.Constant.CHATTYPE_CHATROOM);
                        }
        ...
        connectionListener = new EMConnectionListener() {
            ...
            public void onDisconnected(int error) {
                ...
                if (error == EMError.USER_REMOVED) {
                    onUserException(byou.yadun.wallet.Chat.Constant.ACCOUNT_REMOVED);
                    //账号在另一个登录
                } else if (error == EMError.USER_LOGIN_ANOTHER_DEVICE) {
                    onUserException(byou.yadun.wallet.Chat.Constant.ACCOUNT_CONFLICT);
                } else if (error == EMError.SERVER_SERVICE_RESTRICTED) {
                    onUserException(byou.yadun.wallet.Chat.Constant.ACCOUNT_FORBIDDEN);
                } else if (error == EMError.USER_KICKED_BY_CHANGE_PASSWORD) {
                    onUserException(byou.yadun.wallet.Chat.Constant.ACCOUNT_KICKED_BY_CHANGE_PASSWORD);
                } else if (error == EMError.USER_KICKED_BY_OTHER_DEVICE) {
                    onUserException(byou.yadun.wallet.Chat.Constant.ACCOUNT_KICKED_BY_OTHER_DEVICE);
                }
            }
    ...
    class MyGroupChangeListener implements EMGroupChangeListener {
        ...
        public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
            ...
            broadcastManager.sendBroadcast(new Intent(byou.yadun.wallet.Chat.Constant.ACTION_GROUP_CHANAGED));
        }
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            ...
            broadcastManager.sendBroadcast(new Intent(byou.yadun.wallet.Chat.Constant.ACTION_CONTACT_CHANAGED));
            showToast("onContactAdded:" + username);
        }

        @Override
        public void onContactDeleted(String username) {
            Map<String, EaseUser> localUsers = ByHelper.getInstance().getContactList();
...
//                break;
    ...
    protected void onUserException(String exception) {
        ...
        PreferenceUtil.commitInt("hadlogined", 0);
...
//        return user;
        ...
        if (username.equals(EMClient.getInstance().getCurrentUser())) {
            user = new EaseUser(username);
//            user.setAvatar("http://www.qqxoo.com/uploads/allimg/160208/19291Q227-3.jpg");
            user.setAvatar(avatar[PreferenceUtil.getInt("userheadimage", 0)]);
            user.setNick(SPUtils.getNickname(MyApplication.applicationContext, "nick"));
            return user;
        }
//        if (user==null && getRobotList()!=null){
//            user=getRobotList().get(username);
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    //接收并处理扩展消息
                    String userName = message.getStringAttribute(byou.yadun.wallet.Chat.Constant.USER_NAME, "");
                    String userId = message.getStringAttribute(byou.yadun.wallet.Chat.Constant.USER_ID, "");
                    String userPic = message.getStringAttribute(byou.yadun.wallet.Chat.Constant.HEAD_IMAGE_URL, "");
...
//                    }
            ...
            public void onMessageRecalled(List<EMMessage> messages) {
                for (EMMessage msg : messages) {
                    ...
                    msgNotification.setAttribute(byou.yadun.wallet.Chat.Constant.MESSAGE_TYPE_RECALL, true);
    ...
    public ByModel getModel() {
        return (ByModel) demoModel;
    }
