package byou.yadun.wallet.parse;
...
public class UserProfileManager {
	 ...
	 */
	private List<ByHelper.DataSyncListener> syncContactInfosListeners;
	...
	public synchronized boolean init(Context context) {
		...
		syncContactInfosListeners = new ArrayList<ByHelper.DataSyncListener>();
		sdkInited = true;
		return true;
	}

	public void addSyncContactInfoListener(ByHelper.DataSyncListener listener) {
		if (listener == null) {
			return;
		}
	...
	public void removeSyncContactInfoListener(ByHelper.DataSyncListener listener) {
		if (listener == null) {
			return;
		}
		if (syncContactInfosListeners.contains(listener)) {
	...
	public void asyncFetchContactInfosFromServer(List<String> usernames, final EMValueCallBack<List<EaseUser>> callback) {
		...
		ParseManager.getInstance().getContactInfos(usernames, new EMValueCallBack<List<EaseUser>>() {
			...
			public void onSuccess(List<EaseUser> value) {
				...
				// return immediately
				if (!ByHelper.getInstance().isLoggedIn()) {
					return;
				}
				if (callback != null) {
	...
	public void notifyContactInfosSyncListener(boolean success) {
		for (ByHelper.DataSyncListener listener : syncContactInfosListeners) {
	...
	private void setCurrentUserAvatar(String avatar) {
		...
		SPUtils.setUser_image(MyApplication.applicationContext,avatar);//保存本人头像
