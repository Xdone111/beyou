...
//    			tvUsername.setText(EMClient.getInstance().getCurrentUser());
    			tvUsername.setText(PreferenceUtil.getString("usernickName",new PreferenceUtil().getUser("account")));
	...
	public void asyncFetchUserInfo(String username){
		ByHelper.getInstance().getUserProfileManager().asyncGetUserInfo(username, new EMValueCallBack<EaseUser>() {

			@Override
			public void onSuccess(EaseUser user) {
				if (user != null) {
				    ByHelper.getInstance().saveContact(user);
				    if(isFinishing()){
				        return;
				    }
					tvNickName.setText(user.getNick());
					if(!TextUtils.isEmpty(user.getAvatar())){
//						 Glide.with(UserProfileActivity.this).load(user.getAvatar()).placeholder(R.drawable.em_default_avatar).inteno(headAvatar);
...
//			@Override
//			public void run() {
				PreferenceUtil.commitString("usernickName",nickName);
				dialog.dismiss();
							Toast.makeText(UserProfileActivity.this, getString(R.string.toast_updatenick_success), Toast.LENGTH_SHORT)
									.show();
		finish();
//				boolean updatenick = ByHelper.getInstance().getUserProfileManager().updateCurrentUserNickName(nickName);
...
//		}).start();
	...
	private void uploadUserAvatar(final byte[] data) {
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				final String avatarUrl = ByHelper.getInstance().getUserProfileManager().uploadUserAvatar(data);
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						dialog.dismiss();
