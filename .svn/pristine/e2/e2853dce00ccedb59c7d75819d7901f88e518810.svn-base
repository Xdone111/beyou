...
public class LogoConfig {
	...
	public Bitmap modifyLogo(Bitmap bgBitmap, Bitmap logoBitmap) {
		...
		if (logoBitmap.isRecycled()) return null;

		//读取背景图片，并构建绘图对象
		int bgWidth = bgBitmap.getWidth();
		int bgHeigh = bgBitmap.getHeight();
		//通过ThumbnailUtils压缩原图片，并指定宽高为背景图的3/4
		Bitmap bitmap = ThumbnailUtils.extractThumbnail(logoBitmap,bgWidth*3/4, bgHeigh*3/4, ThumbnailUtils.OPTIONS_RECYCLE_INPUT);
		Bitmap cvBitmap = Bitmap.createBitmap(bgWidth, bgHeigh, Config.ARGB_8888);
		Canvas canvas = new Canvas(cvBitmap);
		// 开始绘制图片
		canvas.drawBitmap(bgBitmap, 0, 0, null);
		canvas.drawBitmap(bitmap,(bgWidth - bitmap.getWidth()) /2,(bgHeigh - bitmap.getHeight()) / 2, null);
