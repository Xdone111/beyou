...
public class ReceiveAdapter2 extends BaseAdapter{
    private Context mContext;
    private List<TradeNoteResponse.DataBeanXXX.ListBean.DataBean> data;
    private SimpleDateFormat mDateFormat;

    public ReceiveAdapter2(Context context, List<TradeNoteResponse.DataBeanXXX.ListBean.DataBean> data) {
        mContext = context;
        this.data = data;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ReceiveAdapter.ViewHolder holder;
        if(convertView==null){
        ...
        if(data.get(position).getZc_type().equals("0")){
            ...
            if(data.get(position).getStatus()==1){
                holder.state.setText(R.string.trade_tx_success);
                holder.state.setTextColor(0XFF23a800);
            }else{
                holder.state.setText(R.string.trade_tx_ing);
                holder.state.setTextColor(0xFFFF0000);
            }
        }
