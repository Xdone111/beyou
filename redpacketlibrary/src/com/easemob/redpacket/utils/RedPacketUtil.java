...
//            redPacketInfo.toNickName = toUserName;
    ...
    public static void openRedPacket(final FragmentActivity activity, final int chatType, final EMMessage message, final String toChatUsername, final EaseChatMessageList messageList) {
        Toast.makeText(activity, "拆红包咯"+toChatUsername, Toast.LENGTH_SHORT).show();
        if (toChatUsername.equals(message.getFrom())){
            Toast.makeText(activity, "本人在拆红包", Toast.LENGTH_SHORT).show();
        }else {
            Toast.makeText(activity, "对方在拆红包"+message.toString()+messageList.toString(), Toast.LENGTH_SHORT).show();
